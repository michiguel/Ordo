...
A combination of switches \swtch{-d <value>} and \swtch{-k <deviation>} may provide a prior information to Ordo to calculate the \textit{draw rate}.
Again, when as the number of games increases, this information will have less contribution.
Ordo assumes a Gaussian distribution centered in \swtch{<value>} with a standard \swtch{<deviation>}.
...
In certain cases, the use may not want to include in the rating certain players with very few games played. 
...
when user provides Ordo with either \textit{loose anchors}, \textit{relative anchors}, \textit{white advantage} uncertainty, or a \textit{draw rate} uncertainty the calculation is performed by Maximum Likelihood.
